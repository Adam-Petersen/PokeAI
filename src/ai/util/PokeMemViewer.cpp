...
Pokemon* PokeMemViewer::initPokemon(uint16_t start_addr, PokeOwner owner, bool in_battle) {
  ...
  mon->setType1(gbReadMemory(addr++));
  mon->setType2(gbReadMemory(addr++)); // SEE WHAT THIS IS WHEN ONE TYPE POKEMON
  addr++; // skip catch rate/held item

  int num_moves = 0;
  uint16_t temp_addr = addr;
  for (int i = 0; i < 4; i++) {
    if(gbReadMemory(temp_addr++) != 0) {
      num_moves++;
    }
  }

  MoveInstance* move_instances = new MoveInstance[num_moves];
  for (int i = 0; i < 4; i++) {
    if (i < num_moves) {
      MoveInstance mi;
      mi.id = gbReadMemory(addr++);
      move_instances[i] = mi;
    } else {
      addr++;
    }
  ...
  if (in_battle) {
    ...
    mon->setMaxHp(read2BytesAndInc(&addr));
    ...
    mon->setStats(attack, defense, speed, special);
    mon->setCatchRate(gbReadMemory(0xD007));
  }

  for (int i = 0; i < 4; i++) {
    if (i < num_moves) {
      move_instances[i].pp = gbReadMemory(addr++);
    } else {
      addr++;
    }
  }
  mon->setMoveInstances(num_moves, move_instances);
...
bool PokeMemViewer::isPokemonOwned(uint8_t index) {
  uint8_t dex_no = Pokemon::index_to_dex_map[index];
